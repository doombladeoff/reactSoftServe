...
import { getTheme, getWords, init } from "../../store/dbUtils";
...
function AllWords({ navigation }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await init();
                const fetchedWords = await getWords(data => dispatch(wordsLearningActions.setInitial(data)));
                dispatch(wordsLearningActions.setInitial(fetchedWords));
                const theme = await getTheme((theme) => dispatch(themeActions.setInitial(theme)))
                dispatch(themeActions.setInitial(theme));
            } catch (err) {
