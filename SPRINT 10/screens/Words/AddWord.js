...
function AddWord({ navigation }) {
    ...
    const [hasPermission, requestPermission] = ImagePicker.useMediaLibraryPermissions();
    ...
    function onAdd() {
        image ? dispatch(wordsLearningActions.addWord({
            ...wordData,
            image
    ...
    const openImagePickerAsync = async () => {
        if (hasPermission?.status !== 'granted') {
            ...
            if (permissionResponse.granted === true || permissionResponse.status === 'granted')
                await addImageToWord();
    ...
    return (
        <>
            <View style={{ justifyContent: 'center', alignItems: 'center', padding: 15 }}>
                <Pressable
                    onPress={hasPermission?.granted === false ? null : openImagePickerAsync}
                    disabled={!wordData?.word}
                    style={{ justifyContent: 'center', alignItems: 'center' }}
                >
                    <ImageViewer
                        placeholderImageSource={require("../../assets/no-image.png")}
                        selectedImage={image}
                        width={125}
                        height={125}
                    />
                    {!image && <Text style={{ color: colors.fontMain }}>No image taken yet.</Text>}
                </Pressable>
            </View>
